...
import { Upload, Trash2, ArrowUp, ArrowDown, Loader2, Eye, EyeOff, Smartphone, Monitor } from "lucide-react";
...
export function HeroManager({
  ...
  async function move(id: string, dir: -1 | 1) {
    ...
    setSlides(reordered);
    // Persist new order indices for both swapped slides
    await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add slide */}
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-slate-900 mb-3">Add a hero image</h2>
        <button
        ...
        >
          ...
          <span className="text-sm font-medium">{busy ? "Working…" : "Click to upload a new slide"}</span>
      ...
      {/* Slides */}
      {slides.length === 0 ? (
        <p className="text-sm text-slate-500 text-center py-8">No slides yet — the site shows the default hero image until you add one.</p>
      ) : (
              ...
              <div key={s.id} className="bg-white border border-slate-200 rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => move(s.id, -1)} disabled={i === 0} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-30 cursor-pointer" aria-label="Move up"><ArrowUp className="h-4 w-4" /></button>
                    <button onClick={() => move(s.id, 1)} disabled={i === slides.length - 1} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-30 cursor-pointer" aria-label="Move down"><ArrowDown className="h-4 w-4" /></button>
                    <button onClick={() => patchSlide(s.id, { isActive: !s.isActive })} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 cursor-pointer" aria-label="Toggle visibility">{s.isActive ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}</button>
                    <button onClick={() => deleteSlide(s.id)} className="p-1.5 rounded-lg text-rose-500 hover:bg-rose-50 cursor-pointer" aria-label="Delete"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div onClick={(e) => setFocal(s.id, "desktop", e)} className="relative aspect-[16/9] w-full rounded-lg overflow-hidden border border-slate-200 cursor-crosshair">
                  ...
                  <div>
                    ...
                    <div onClick={(e) => setFocal(s.id, "mobile", e)} className="relative aspect-[9/16] w-full max-w-[180px] mx-auto rounded-lg overflow-hidden border border-slate-200 cursor-crosshair">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
...
function MobileImageButton({
  ...
  return (
    <div className="flex items-center justify-center gap-3 mt-2">
      <button onClick={() => ref.current?.click()} className="text-xs text-slate-600 underline hover:text-slate-900 cursor-pointer">
        {hasMobile ? "Replace mobile image" : "Use a different image on mobile"}
      </button>
      {hasMobile && (
        <button onClick={onClear} className="text-xs text-rose-500 underline hover:text-rose-700 cursor-pointer">Remove</button>
      )}
