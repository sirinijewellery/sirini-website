// Assemble dist/SFJ_World.html — one self-contained file, zero runtime requests.
...
// Images → SFJ_ASSETS map
const imgDir = join(here, 'assets', 'web');
const assets = {};
for (const file of readdirSync(imgDir).sort()) {
  if (!file.endsWith('.jpg')) continue;
  const slug = file.replace(/\.jpg$/, '');
  assets[slug] = `data:image/jpeg;base64,${readBin(join('assets/web', file)).toString('base64')}`;
}
const assetJs = `window.SFJ_ASSETS=${JSON.stringify(assets)};`;
...
const out = join(here, 'dist', 'SFJ_World.html');
writeFileSync(out, html);
console.log(`[build] dist/SFJ_World.html — ${(html.length / 1024 / 1024).toFixed(2)} MB, ${Object.keys(assets).length} images inlined`);
