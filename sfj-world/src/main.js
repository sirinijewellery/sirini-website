...
/* ————— 1 · Content (verbatim from the SFJ company profile) ————— */

const GALLERY = [
  { slug: 'mehandi-brass',   caption: 'Mehandi Plated Reverse Jewellery', material: 'Made in Brass' },
  { slug: 'mehandi-ad-zinc', caption: 'Mehandi Plated Reverse AD Jewellery', material: 'Made in Zinc Alloy' },
  { slug: 'heritage',        caption: 'Heritage Jewellery', material: '' },
  { slug: 'karat-kundan',    caption: 'Karat Plated Kundan Jewellery', material: '' },
  { slug: 'temple',          caption: 'Temple Jewellery', material: '' },
  { slug: 'moissanite',      caption: 'Moissanite Jewellery', material: '' },
  { slug: 'antique-kundan',  caption: 'Antique Kundan Jewellery', material: '' },
  { slug: 'idols-figurines', caption: 'Idols and Figurines', material: '' },
  { slug: 'sindoor-dabbi',   caption: 'Sindoor Dabbis', material: '' },
  { slug: 'mens-accessories',caption: 'Mens Accessories', material: '' },
  { slug: 'mangalsutra',     caption: 'Mangalsutra', material: '' },
  { slug: 'maharashtrian',   caption: 'Maharashtrian Jewellery', material: '' },
];

const CATEGORIES = [
  ['Bridal & Wedding Sets', 'Complete sets — necklace, earrings, maang tika, bangles, and accessories for wedding occasions.'],
  ['Necklace & Choker Sets', 'Statement pieces in antique gold, kundan, and polki finishes.'],
  ['Earrings', 'Jhumkas, chandeliers, studs, hoops — wide range across styles and price points.'],
  ['Bangles & Bracelets', 'Gold-plated bangles, bangle sets, kadas, and designer bracelets.'],
  ['Maang Tikka & Matha Patti', 'Forehead ornaments for bridal and festive wear.'],
  ['Anklets (Payal)', 'Traditional and contemporary anklet designs.'],
  ['Armlets & Baju Band', 'Upper arm ornaments for bridal and classical costume styling.'],
  ['Nose Rings (Nath)', 'Various sizes and stone settings.'],
  ['Waist Belts (Kandora)', 'Decorative waist belts for bridal and festive styling.'],
  ['Pendant Sets', 'Pendant and chain combinations for everyday and occasion wear.'],
  ['Finger Rings', 'Statement and adjustable designs across styles.'],
  ['Hair Accessories', 'Juda pins, clips, and traditional styling accessories.'],
];

const PILLARS = [
  ['In-House Manufacturing', 'Every piece designed and made at our Mumbai facility. No middlemen, full quality control.'],
  ['Antique Jewellery Expertise', '15+ years specialising in antique, kundan, and polki finishes the market trusts.'],
  ['Wholesale Pricing', 'Manufacturer-direct rates — best value for retailers and distributors.'],
  ['Wide Variety', '10000+ SKUs across 12 categories, bridal to everyday, festive to casual.'],
  ['Quality Assurance', 'Rigorous in-house QC from raw material selection to final finishing.'],
  ['Fast Dispatch', 'Reliable Pan-India shipping and export fulfilment from Mumbai.'],
];

const STATS = [
  ['Est. 2010', '15+ YEARS IN BUSINESS'],
  ['Mumbai', 'MANUFACTURING BASE'],
  ['10000+', 'SKUs · ACTIVE CATALOGUE'],
  ['Pan-India', 'WHOLESALE REACH'],
];
...
const ZONE_Z = [
  ...
  ['prologue',  -6,   -26, null],
  ['house',     -26,  -56, 'The House'],
  ['glance',    -56,  -94, 'At a Glance'],
  ['atelier',   -94,  -140,'The Atelier'],
  ['catalogue', -140, -180,'The Catalogue'],
  ['gallery',   -180, -310,'The Gallery'],
  ['reach',     -310, -342,'The Reach'],
  ['pillars',   -342, -374,'Six Pillars'],
  ['ascent',    -374, -428, null],
  ['ring',      -428, -447,'Contact'],
];
...
function fillLists() {
  ...
  const pil = $('#pillarsList');
  if (pil) {
    pil.innerHTML = PILLARS.map(([n, d]) =>
      `<li><span class="fl-name">${n}</span><span class="fl-desc">${d}</span></li>`).join('');
  }
}

function enterFallback(reason) {
  console.warn('[SFJ] falling back to editorial mode:', reason);
  ...
  if (extras) {
    extras.hidden = false;
    const logo = $('#fbLogo');
    if (logo && window.SFJ_ASSETS) logo.src = window.SFJ_ASSETS.logo;
    const grid = $('#fbGrid');
    if (grid && window.SFJ_ASSETS) {
      grid.innerHTML = GALLERY.map((g) =>
        `<figure><img loading="lazy" src="${window.SFJ_ASSETS[g.slug]}" alt="${g.caption}"><figcaption>${g.caption}${g.material ? ' · ' + g.material : ''}</figcaption></figure>`).join('');
    }
  }
...
if (!window.THREE || !window.SFJ_ASSETS) { enterFallback('runtime payload missing'); return; }
...
canvas.addEventListener('webglcontextlost', (e) => { e.preventDefault(); running = false; enterFallback('graphics context lost'); });

/* ————— 5 · Palette, scene, camera ————— */

const GOLD = 0xd8b36a, GOLD_BRIGHT = 0xf0ce8c, IVORY = 0xf2e9d8, OBSIDIAN = 0x16110c, FOGCOL = 0x0d0a07;
...
/* Environment reflections: procedural warm "softbox studio" equirect. */
function buildEnvironment() {
  ...
  soft(200, 150, 190, 'rgba(255,225,176,A)', '0.95');
  soft(700, 120, 240, 'rgba(255,240,214,A)', '0.8');
  soft(500, 320, 300, 'rgba(214,164,96,A)', '0.35');
  soft(900, 260, 150, 'rgba(255,214,150,A)', '0.5');
...
const matObsidian = new THREE.MeshStandardMaterial({ color: 0x1a1410, metalness: 0.85, roughness: 0.42, envMapIntensity: 1.1 });
const matInk = new THREE.MeshStandardMaterial({ color: 0x0e0b08, metalness: 0.6, roughness: 0.35, envMapIntensity: 0.7 });

const FONT_SERIF = '"Cormorant Garamond", "Palatino Linotype", Georgia, serif';
const FONT_SANS = '"Jost", "Segoe UI", sans-serif';

/* Engraved plaque texture: dark stone, hairline border, gold serif. */
function plaqueTexture({ w = 1024, h = 448, title, body, titleSize = 92, bodySize = 40, pad = 70, align = 'left' }) {
  return canvasTexture(w, h, (g) => {
    g.fillStyle = '#0e0b07';
    g.fillRect(0, 0, w, h);
    const grad = g.createLinearGradient(0, 0, w, h);
    grad.addColorStop(0, 'rgba(255,225,170,0.05)');
    grad.addColorStop(1, 'rgba(0,0,0,0.14)');
    g.fillStyle = grad;
    g.fillRect(0, 0, w, h);
    g.strokeStyle = 'rgba(216,179,106,0.4)';
    g.lineWidth = 2;
    ...
    if (body) {
      g.shadowBlur = 0;
      g.fillStyle = 'rgba(223,213,190,0.82)';
...
function textPlane(text, { size = 220, font = FONT_SERIF, weight = 500, color = '#e8c987', width = 1024, height = 512, letterSpacing = 0, planeH = 2 }) {
...
function threadMaterial() {
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      varying vec2 vUv; varying float vDepth; varying vec3 vNormalW;
      void main() {
        vec3 base = vec3(0.62, 0.47, 0.24);
        vec3 bright = vec3(1.0, 0.86, 0.58);
...
const frameGroups = []; // gallery frames for hover/click
const photoMeshes = [];
...
function buildThreshold() {
  ...
  // molten droplet — the origin of the thread
  const droplet = new THREE.Mesh(new THREE.SphereGeometry(0.42, 32, 24), matGoldBright.clone());
  droplet.material.emissive = new THREE.Color(0xffc978);
  droplet.material.emissiveIntensity = 1.4;
  droplet.position.set(0, 1.9, -14);
  g.add(droplet);
  const halo = makeGlow(4.6, 0.5);
  halo.position.copy(droplet.position);
  g.add(halo);
  const spark = makeGlow(1.1, 0.9, 0xfff2d8);
  spark.position.copy(droplet.position);
  g.add(spark);
  // gyroscope setting — two fine rings orbiting the droplet like a jewel mount
  ...
  const orbitB = new THREE.Mesh(new THREE.TorusGeometry(1.5, 0.011, 8, 84), matGold);
...
function buildThread() {
  // thread wanders beside the camera path, then coils into the finale ring
  const pts = [];
...
function buildHouse() {
  ...
  // slabs keep to the left, behind the copy panel — the portrait owns the right
  [[-34, -1], [-40, -1], [-47, -1], [-53, -1]].forEach(([z, side], i) => {
    const { p, right } = frameBasis(z);
    const slab = new THREE.Mesh(slabGeo, matObsidian);
...
function buildGlance() {
  ...
  STATS.forEach(([num, label], i) => {
    ...
    const ring = new THREE.Mesh(ringGeo, matGoldBright);
    const disc = new THREE.Mesh(discGeo, new THREE.MeshBasicMaterial({
      map: canvasTexture(512, 512, (gc) => {
        ...
        gc.shadowBlur = 0;
        gc.fillStyle = 'rgba(223,213,190,0.85)';
...
function buildAtelier() {
  ...
  // a tunnel of slowly turning gates — the lathe of light the camera flies through
  ...
  gateZs.forEach((z, i) => {
    ...
    const ring = new THREE.Mesh(new THREE.TorusGeometry(r, 0.055, 10, 110), i % 2 ? matGold : matGoldBright);
...
/* Shared museum frame: backboard, photo, gold border, caption plinth, spotlight. */
let coneTex = null;
...
/* Model photography walks with the visitor — one editorial frame per chamber. */
const EDITORIALS = [
  { slug: 'karat-kundan',    zone: 'house',     z: -45,    side: 1,  lateral: 5.6, y: 1.0,  s: 1.18 },
  { slug: 'mehandi-brass',   zone: 'glance',    z: -91,    side: -1, lateral: 4.8, y: 0.75, s: 1.05 },
  { slug: 'mehandi-ad-zinc', zone: 'atelier',   z: -126,   side: 1,  lateral: 7.0, y: 1.0,  s: 1.15 },
  { slug: 'mangalsutra',     zone: 'catalogue', z: -178.5, side: -1, lateral: 5.0, y: 0.75, s: 1.1 },
  { slug: 'moissanite',      zone: 'reach',     z: -336,   side: 1,  lateral: 5.6, y: 0.9,  s: 1.1 },
  { slug: 'heritage',        zone: 'pillars',   z: -349,   side: 1,  lateral: 6.2, y: 0.85, s: 1.1 },
];
const editorialGroups = [];
function buildEditorials() {
  for (const ed of EDITORIALS) {
    const item = GALLERY.find((gi) => gi.slug === ed.slug);
...
function buildReach() {
  ...
  const nodeMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        gl_FragColor = vec4(1.0, 0.84, 0.55, a * 0.9);
      }`,
  });
  ...
  g.add(beacon);
  // shipments of light travelling outward from Mumbai along the filaments
  ...
  const pkMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        float d = length(gl_PointCoord - 0.5);
        gl_FragColor = vec4(1.0, 0.87, 0.6, smoothstep(0.5, 0.1, d) * vA * 0.8);
      }`,
  });
...
function buildPillars() {
  ...
  PILLARS.forEach(([name, body], i) => {
    const a = -Math.PI / 2 + (i / PILLARS.length) * TAU;
    ...
    grp.add(pillar);
    [3.3, -3.3].forEach((y) => {
      const cap = new THREE.Mesh(capGeo, matGoldBright);
      cap.position.y = y;
      grp.add(cap);
    });
    // engraved face
    const plaque = new THREE.Mesh(plaqueGeo, new THREE.MeshBasicMaterial({
      map: canvasTexture(640, 856, (gc) => {
        ...
        gc.strokeStyle = 'rgba(216,179,106,0.45)';
        gc.lineWidth = 3;
        ...
        gc.shadowBlur = 0;
        gc.fillStyle = 'rgba(233,223,200,0.95)';
  ...
  g.add(halo);
  // a golden stage ring binding the six pillars together
...
function buildAscent() {
  ...
  const dawn = makeGlow(70, 0.34, 0xffe6c0);
  dawn.position.set(0, 30, -486);
  g.add(dawn);
  const dawn2 = makeGlow(26, 0.4, 0xfff3dd);
...
function buildRing() {
  ...
  const engraving = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({
      ...
      map: canvasTexture(1024, 1024, (gc) => {
        ...
        const msg = 'SALONI FASHION JEWELLERY · MUMBAI · EST. 2010 · ';
        const chars = [...msg];
        chars.forEach((ch, i) => {
  ...
  g.add(heart);
  // concentric echo halos — visible through the fog during the ascent
  [[4.8, 0.3], [7.0, 0.16]].forEach(([r, op]) => {
    const echo = new THREE.Mesh(
      new THREE.TorusGeometry(r, 0.02, 8, 100),
      new THREE.MeshBasicMaterial({ color: GOLD_BRIGHT, transparent: true, opacity: op })
    );
...
function buildDust(count, sizeMul = 1, alphaMul = 1, driftMul = 1) {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        gl_FragColor = vec4(1.0, 0.85, 0.58, a * 0.8);
      }`,
  });
...
const hemi = new THREE.HemisphereLight(0x2c2216, 0x0a0704, 0.82);
scene.add(hemi);

const roamers = [
  new THREE.PointLight(0xffd9a0, 0, 46, 2),
  new THREE.PointLight(0xffd9a0, 0, 46, 2),
];
...
const LIGHT_ANCHORS = [
  ...
  { id: 'reach', pos: () => V3(-8, 6.5, -330) },
  { id: 'pillars', pos: () => V3(0, 5.5, -361) },
...
function syncCopy(p) {
  for (const c of copyEls) {
    ...
    if (c.zone.id === 'threshold') {
      ...
      oOut = 1;
    } else if (c.zone.id === 'gallery' || c.zone.id === 'catalogue' || c.zone.id === 'pillars') {
...
function fillLightbox() {
  ...
  lbImg.src = window.SFJ_ASSETS[it.slug];
  lbImg.alt = it.caption;
  lbCaption.textContent = it.caption + (it.material ? ' · ' + it.material : '');
}
...
function loadTextures() {
  ...
  const jobs = GALLERY.map((g) => new Promise((res, rej) => {
    loader.load(window.SFJ_ASSETS[g.slug], (tex) => {
      tex.encoding = THREE.sRGBEncoding;
...
function loadFonts() {
  ...
  const wanted = [
    ...
    'italic 500 100px "Cormorant Garamond"',
    '300 100px "Jost"',
    '400 100px "Jost"',
  ];
  return Promise.race([
...
function tick(now) {
  ...
  // frame hover ease (gallery + chamber editorials)
...
async function boot() {
  try {
    ...
    buildDust(Math.round(dustBase / 3), 3.1, 0.32, 0.45); // soft distant bokeh layer
    resize();
    // honour deep link
    ...
    console.error('[SFJ] boot failed', err);
