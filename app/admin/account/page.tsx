...
export default async function AdminAccountPage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { username: true, email: true, name: true, createdAt: true, isAdmin: true },
  });
  if (!user) redirect("/login");

  const realEmail = user.email.endsWith("@sirini.local") ? null : user.email;
  ...
  return (
    <div className="p-4 md:p-10 max-w-2xl">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <dd className="text-slate-900 font-medium break-all">{realEmail ?? "—"}</dd>
          </div>
          <div>
      ...
      <AccountForm initialUsername={user.username ?? ""} initialName={user.name ?? ""} initialEmail={user.email} />
    </div>
  );
}
