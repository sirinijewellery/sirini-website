...
export default async function ShippingPage() {
  const [content, time] = await Promise.all([getShipping(), getShippingTime()]);
  // Sections by index: 0 = Shipping Policy, 1 = Return Policy, 2 = Refunds.
  const shippingSection = content.sections[0];
  const returnSection = content.sections[1];
  const refundSection = content.sections[2];
      ...
      <div className="space-y-10 font-sans text-sm text-muted-foreground leading-relaxed">
        ...
        <section className="space-y-4">
          <h2 className="font-display text-2xl font-light text-foreground">
            {shippingSection?.heading ?? "Shipping Policy"}
          </h2>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: "🚚", title: "Free Shipping", desc: "On all orders across India. No minimum order value." },
              { icon: "⏱️", title: `${time.deliveryDays} Business Days`, desc: "Standard delivery time after order confirmation." },
          ...
          <p>{shippingSection?.body}</p>
        </section>

        {/* Returns */}
        <section className="space-y-4">
          <h2 className="font-display text-2xl font-light text-foreground">
            {returnSection?.heading ?? "Return Policy"}
          </h2>
          <p>
            We accept returns within{" "}
            <strong className="text-foreground">{time.returnDays} days</strong> of delivery
          ...
          <div className="space-y-3">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Items returned after {time.returnDays} days</li>
        ...
        <section className="space-y-3">
          <h2 className="font-display text-2xl font-light text-foreground">
            {refundSection?.heading ?? "Refunds"}
          </h2>
          <p>
            Once your return is received and inspected, we will process your refund within{" "}
            <strong className="text-foreground">{time.refundDays} business days</strong>. Refunds are credited
